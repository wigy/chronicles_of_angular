describe('module coa.store, Lookup class', function() {
    ...
    it('can search for more than one field', function() {
        ...
        var res2 = q2.find(haystack1);
        expect(res2.length).toBe(0);
    });

    it('finds everyhing by default', function() {
        var q = new Lookup();
        expect(q.find(haystack1).length).toBe(4);
        expect(q.find(haystack2).length).toBe(4);

        var q2 = new Lookup({});
        expect(q2.find(haystack1).length).toBe(4);
        expect(q2.find(haystack2).length).toBe(4);
    });
});
